...
public class SQLArticoloDAO implements ArticoloDAO<SQLException>{
    ...
    public List<Articolo> doRetrieveNewArrivals(String sex) throws SQLException{
        try(Connection con = ConPool.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT a.*, s.Quantita, ta.id_nome, c.nome_colore, " +
    ...
    public List<String> doRetrieveProductByNome() throws SQLException {
        try(Connection con = ConPool.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT DISTINCT a.nome_articolo as nome " +
                    "FROM articolo a;")){
                ResultSet rs = ps.executeQuery();
    ...
    @Override
    public List<Articolo> searchPagination(List<Condition> conditions, Paginator paginator, boolean latest) throws SQLException{
        try (Connection con = ConPool.getConnection()){
    ...
    public double maxPrice() throws SQLException {
        try (Connection con = ConPool.getConnection()) {
            try ( PreparedStatement ps = con.prepareStatement("SELECT MAX(Prezzo) as price " +
                    "FROM articolo a;")){
                ResultSet rs = ps.executeQuery();
                if (rs.next())
    ...
    public double minPrice() throws SQLException {
        try (Connection con = ConPool.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT MIN(Prezzo) as price " + "FROM articolo a;")){
                ResultSet rs = ps.executeQuery();
                if (rs.next())
                    return rs.getDouble("price");
                return 0;
    ...
    public boolean reduceSize(Articolo articolo) throws SQLException {
        try(Connection con = ConPool.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("UPDATE size s INNER JOIN articolo a on s.ID_articolo = a.ID_articolo " +
